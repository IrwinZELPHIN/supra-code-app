...
const Questionnaire = () => {
  ...
  const isLastRealQuestion = currentIndex === questions.length - 1; // Q28 est la dernière vraie question
  const showFinalPage = currentIndex >= questions.length; // Page finale après Q28

  // Calcul des blocs ordonnés (sans Q0)
  ...
  const isEndOfBloc = useMemo(() => {
    if (isLast || currentQuestion.id === 'Q0' || showFinalPage) return false;
    const nextQuestion = questions[currentIndex + 1];
    return nextQuestion && currentQuestion.bloc !== nextQuestion.bloc;
  }, [currentIndex, currentQuestion, isLast, showFinalPage]);
  ...
  const handleNext = () => {
    ...
    if (isEndOfBloc && !showFinalPage) {
      ...
      setCurrentIndex(questions.length); // Index au-delà des questions = page finale
